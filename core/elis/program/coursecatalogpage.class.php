...
class coursecatalogpage extends pm_page {
    ...
    function display_add() { // action_add
        ...
        if ($classes = pmclass_get_listing($sort, $dir, 0, 0, '', '', $crsid, true)) {
...
//s=crscat&amp;section=curr&amp;clsid=
    ...
    function include_js() {
        ...
        echo html_writer::tag('style', '@import url("'.$CFG->wwwroot.'/lib/yui/2.9.0/build/assets/skins/sam/datatable.css");');
...
class yui_table extends display_table {
    ...
    function get_yui_columns() {
        ...
        foreach($this->columns as $column_id => $val) {
            ...
            $s .= "{key:\"$column_id\", sortable:".((!isset($val['sortable']) || !empty($val['sortable'])) ? 'true' : 'false').", label:\"$column_label\", resizeable:true";
            if(!empty($this->yui_formatters[$column_id])) {
                $s .= ', formatter:' . $this->yui_formatters[$column_id];
            }
            if (!empty($this->yui_sorters[$column_id])) {
                $s .= ', sortOptions:{sortFunction:'.$this->yui_sorters[$column_id].'}';
            }
            $s .= "},\n";
        }
