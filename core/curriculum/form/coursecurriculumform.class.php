...
class coursecurriculumbaseform extends cmform {
    ...
    public function definition() {
        ...
        $mform->addElement('advcheckbox', 'required', get_string('required', 'block_curr_admin'), null, null, array('0', '1'));
        $mform->setHelpButton('required', array('coursecurriculumform/required', get_string('required', 'block_curr_admin'), 'block_curr_admin'));

        $mform->addElement('text', 'frequency', get_string('frequency', 'block_curr_admin') . ':');
        $mform->setType('frequency', PARAM_INT);
        $mform->addRule('frequency', null, 'maxlength', 64, 'client');
        $mform->setHelpButton('frequency', array('coursecurriculumform/frequency', get_string('frequency', 'block_curr_admin'), 'block_curr_admin'));

        $mform->addElement('select', 'timeperiod', get_string('time_period', 'block_curr_admin') . ':', $this->timeperiod_values);
        $mform->setHelpButton('timeperiod', array('coursecurriculumform/timeperiod', get_string('time_period', 'block_curr_admin'), 'block_curr_admin'));

        $mform->addElement('text', 'position', get_string('curriculumcourse_position', 'block_curr_admin') . ':');
        $mform->setType('position', PARAM_INT);
        $mform->setHelpButton('position', array('coursecurriculumform/position', get_string('position', 'block_curr_admin'), 'block_curr_admin'));
...
//curriculumid, courseid, id, availablePrerequisites, existingPrerequisites
        ...
        $mform->addElement('header', 'curriculumcourseeditform', get_string('edit_course_prerequisites', 'block_curr_admin'));
        $mform->closeHeaderBefore('prerequisiteSave');

        $select =& $mform->addElement('select', 'prereqs', get_string('available_course_prerequisites', 'block_curr_admin'), $data->availablePrerequisites);
        $select->setMultiple(true);

        $select =& $mform->addElement('select', 'sprereqs', get_string('existing_course_prerequisites', 'block_curr_admin'), $data->existingPrerequisites);
        $select->setMultiple(true);

        $mform->addElement('checkbox', 'add_to_curriculum', get_string('add_prereq_to_curriculum', 'block_curr_admin'));

        $group = array();
        $group[] =& $mform->createElement('submit', 'add', get_string('add_prereq', 'block_curr_admin'));
        $group[] =& $mform->createElement('submit', 'remove', get_string('remove_prereq', 'block_curr_admin'));
